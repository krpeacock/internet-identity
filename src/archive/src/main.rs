...
fn store_call_error(call_error: CallErrorInfo) {
    let error_limit = with_config(|config| config.error_buffer_limit) as usize;
...
fn initialize(arg: ArchiveInit) {
    write_config(ArchiveConfig {
        ...
        polling_interval_ns: arg.polling_interval_ns,
        error_buffer_limit: arg.error_buffer_limit,
    });
...
async fn status() -> ArchiveStatus {
    ...
    let config = with_config(|config| ArchiveInit {
        ...
        error_buffer_limit: config.error_buffer_limit,
    });
